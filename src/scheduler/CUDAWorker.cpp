...
CUDAWorker::CUDAWorker() {
    thread_ = std::thread(&CUDAWorker::workerLoop, this);
}
...
TaskHandle CUDAWorker::submit(const Workload& workload, const ExecutionPlan& plan) {
    std::lock_guard<std::mutex> lock(mutex_);
...
TaskResult CUDAWorker::wait(TaskHandle handle) {
...
TaskStatus CUDAWorker::status(TaskHandle handle) {
    std::lock_guard<std::mutex> lock(mutex_);
    auto it = jobs_.find(handle);
...
void CUDAWorker::workerLoop() {
    while (true) {
        ...
        {
            std::lock_guard<std::mutex> lock(mutex_);
            execute_cb = jobs_[handle].workload.cuda_execute;
        }
        ...
        if (execute_cb) {
            ...
            success = false;
            error_msg = "CUDA execute callback not provided / CUDA unavailable";
        }
